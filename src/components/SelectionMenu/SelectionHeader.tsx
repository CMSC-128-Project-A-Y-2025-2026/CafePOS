...
export default function SelectionHeader({ formattedTime }: SelectionHeaderProps) {
    ...
    return (
        <header className="flex w-full items-center justify-between px-8 py-6">
            ...
            <div className="flex items-baseline gap-3 font-black italic tracking-tight drop-shadow-sm">
                <span className="text-[64px] text-[#6290C3]">{formattedTime.split(' ')[0]}</span>
                <span className="text-gray-900 text-[48px]">{formattedTime.split(' ')[1]}</span>
            </div>
        </header>
    );
}