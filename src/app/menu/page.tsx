...
import CategoryFilterPill from './components/CategoryFilterPill';
...
const filterCategories = [{ key: 'all', label: 'all' }, ...menuCategories.map(c => ({ key: c, label: c }))];
  ...
  const handleSaveProduct = (newProduct: Omit<MenuItem, 'id'> | MenuItem) => {
    if ('id' in newProduct) {
      setMenuItems(currentItems => 
        currentItems.map(item => item.id === newProduct.id ? newProduct : item)
      );
  ...
  return (
    <div className={`flex min-h-screen flex-col bg-[#F9F1E9] ${montserrat.className}`}>
      ...
      <div className="p-6 pt-0 flex flex-col flex-1 overflow-hidden">
        ...
        <nav className="mb-4 flex flex-col flex-shrink-0">
        ...
        <div className="relative mb-6 drop-shadow-[0px_2px_4px_rgba(0,0,0,0.25)] flex-shrink-0">
          <input
            type="text"
